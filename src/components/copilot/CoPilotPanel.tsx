...
const CoPilotPanel: React.FC = () => {
  ...
  return (
    <>
      <div
        className={`fixed inset-0 ${isDark ? 'bg-black/40' : 'bg-black/20'} backdrop-blur-sm z-40 transition-opacity`}
        onClick={closePanel}
      />
      ...
      <div
        className={`
          ...
          ${isSoftModern ? 'shadow-[8px_8px_16px_rgba(0,0,0,0.08),-8px_-8px_16px_rgba(255,255,255,0.9)]' : 'shadow-2xl'}
          flex flex-col
...
const CoPilotSettings: React.FC<{ onClose: () => void }> = ({ onClose }) => {
  ...
  return (
    <div className={`px-4 py-3 border-b-2 space-y-3 ${
      isDark
